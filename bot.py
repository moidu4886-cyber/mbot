...
async def start(client, message):
    ...
    # Clear wait state if user starts again
    if message.from_user.id in user_wait:
        del user_wait[message.from_user.id]
...
async def back_home(client, query):
    if query.from_user.id in user_wait:
        del user_wait[query.from_user.id]
...
async def plan_details(client, query):
    ...
    if not p: return await query.answer("❌ Plan not found!")
...
async def ask_unlock(client, query):
    pid = int(query.data.split("_")[1])
    user_wait[query.from_user.id] = pid # Start waiting for code
    await query.message.reply("🔑 **Please send your Unlock Code:**\n(അല്ലെങ്കിൽ തിരിച്ചു പോകാൻ /start അടിക്കുക)")
...
async def handle_text(client, message):
    ...
    if user_id in ADMIN_IDS and user_id in edit_state:
        ...
        if pid == "settings":
            ...
            await message.reply(f"✅ Support ID updated to: {message.text}")
            ...
            await message.reply(f"✅ Plan {pid} updated")
        del edit_state[user_id]
        return

    # USER UNLOCK LOGIC (Strict)
    if user_id in user_wait:
        ...
        del user_wait[user_id] # Task done, stop waiting
    else:
        # If user sends random text without clicking Unlock button
        if user_id not in ADMIN_IDS:
            await message.reply("❓ Please click the **Unlock** button of a plan before sending a code.")
...
async def admin_panel(client, message):
    buttons = [
        ...
        [InlineKeyboardButton("👤 Change Support ID", callback_data="set_global_admin")]
    ]
...
async def set_global_admin(client, query):
    ...
    await query.message.reply("💬 Send new Support Username (eg: @Admin):")
...
async def broadcast(client, message):
    ...
    await message.reply(f"📢 Done. Success: {success}")
